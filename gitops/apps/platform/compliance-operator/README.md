...
- uses `Automatic` install plan approval by default
...
- Automatic approval is used so the operator CRDs are available before the separate `compliance-content` module is enabled.
